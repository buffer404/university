...
enum crud_operation_status find_by_field(FILE *file, uint64_t field_number, char *condition, struct result_list_tuple **result) {
    uint32_t *types;
    size_t size;
    get_types(file, &types, &size);
...
void find_all_by_fields(FILE* file, char* str, uint64_t* fields) {
    struct result_list_tuple* result_list = NULL;
    char* name = "leo";
    find_by_field(file, 0, name, &result_list);
    uint32_t* typess = NULL;
    size_t sizee = NULL;
    get_types(file, &typess, &sizee);
    while (result_list != NULL) {
        if (result_list->value->data[1] > fields[1]) {
            char cur_res[64];
            sprintf(cur_res, "\nTuple id = %d\n", result_list->id);
            strcat(str, cur_res);
            ...
            for (size_t i = 0; i < sizee; i++) {
                if (i == 0) {
                    char* s;
                    read_string_from_tuple(file, &s, sizee, result_list->value->data[i]);
                    printf("string path: %s\n", (s));
                    sprintf(cur_res, "name = %s\n", s);
                    free(s);
                }
                else {
                    sprintf(cur_res, "age = %d\n", result_list->value->data[i]);
                }
                strcat(str, cur_res);
            }
        }
        result_list = result_list->prev;
    }
}

int find_all_by_parent(FILE* file, char* str, int parentId) {
    struct result_list_tuple* result_list = NULL;
    find_by_parent(file, parentId, &result_list);
    uint32_t* typess = NULL;
    size_t sizee = NULL;
    get_types(file, &typess, &sizee);
    while (result_list != NULL) {
